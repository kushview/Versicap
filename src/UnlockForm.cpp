...
{
    OverlayComp (UnlockForm& f, bool hasCancelButton = false)
    ...
    UnlockForm& form;
    Spinner spinner;
    UnlockStatus::UnlockResult result;
...
UnlockForm::UnlockForm (UnlockStatus& s,
...
void UnlockForm::lookAndFeelChanged()
{
    ...
    emailBox.setTextToShowWhenEmpty (TRANS("Username or email address"), labelCol);
...
void UnlockForm::attemptRegistration()
{
    if (unlockingOverlay == nullptr)
    {
        ...
        {
            showBubbleMessage (TRANS ("Please enter a valid username/email address!"), emailBox);
            return;
        }
